...
def D4C(x, fs, f0_object, option=None):
    """ Band-aperiodicity estimation based on D4C """
    f0_low_limit = 47
    fft_size = 2 ** int(np.ceil(np.log2(4 * fs / f0_low_limit + 1)))
    
    f0_low_limit_for_spectrum = 71
    fft_size_for_spectrum = 2 ** int(np.ceil(np.log2(3 * fs / f0_low_limit_for_spectrum + 1)))
    threshold = 0.85
    
    if option:
        threshold = option.get('threshold', threshold)
        fft_size_for_spectrum = option.get('fft_size', fft_size_for_spectrum)
    
    upper_limit = 15000
    frequency_interval = 3000
    
    source_object = f0_object.copy()
    temporal_positions = f0_object['temporal_positions']
    f0 = f0_object['f0']
    ...
    num_aperiodicities = int(min(upper_limit, fs / 2 - frequency_interval) / frequency_interval)
    
    window_length = int(frequency_interval / (fs / fft_size)) * 2 + 1
    window = nuttall(window_length)
    
    aperiodicity = np.zeros((fft_size_for_spectrum // 2 + 1, len(f0)))
    
    frequency_axis = np.arange(fft_size_for_spectrum // 2 + 1) * fs / fft_size_for_spectrum
    coarse_axis = np.concatenate(([0], np.arange(num_aperiodicities) * frequency_interval, [fs / 2]))
    
    for i in range(len(f0)):
        if D4CLoveTrain(x, fs, f0[i], temporal_positions[i], threshold) == 0:
            aperiodicity[:, i] = 1 - 1e-12
            continue
        
        current_f0 = max(f0_low_limit, f0[i])
        coarse_aperiodicity = EstimateOneSlice(x, fs, current_f0, frequency_interval, temporal_positions[i], fft_size, num_aperiodicities, window)
        coarse_aperiodicity = np.maximum(0, coarse_aperiodicity - (current_f0 - 100) * 2 / 100)
        
        aperiodicity[:, i] = 10 ** (np.interp(frequency_axis, coarse_axis, np.concatenate(([-60], -coarse_aperiodicity, [-1e-12]))) / 20)
...
def D4CLoveTrain(x, fs, current_f0, current_position, threshold):
    ...
    power_spectrum[:int(np.ceil(100 / (fs / fft_size))) + 1] = 0.0
...
def EstimateOneSlice(x, fs, current_f0, frequency_interval, current_position, fft_size, num_aperiodicities, window):
    if current_f0 == 0:
        return np.zeros(num_aperiodicities)
    
    waveform = GetWindowedWaveform(x, fs, current_f0, current_position, 2, 1)
    power_spectrum = np.abs(fft(waveform, fft_size)) ** 2
    
    return GetCoarseAperiodicity(power_spectrum, fs, fft_size, frequency_interval, num_aperiodicities, window)

def GetWindowedWaveform(x, fs, current_f0, current_position, half_length, window_type):
    half_window_length = round(half_length * fs / current_f0)
    index = np.round(current_position * fs).astype(int) + np.arange(-half_window_length, half_window_length + 1)
    safe_index = np.clip(index, 0, len(x) - 1)
    segment = x[safe_index]
    time_axis = np.arange(-half_window_length, half_window_length + 1) / fs / half_length
    
    if window_type == 1:
        window = 0.5 * np.cos(np.pi * time_axis * current_f0) + 0.5
    else:
        window = 0.08 * np.cos(2 * np.pi * time_axis * current_f0) + 0.5 * np.cos(np.pi * time_axis * current_f0) + 0.42
    
    return segment * window - window * np.mean(segment * window) / np.mean(window)

def GetCoarseAperiodicity(power_spectrum, fs, fft_size, frequency_interval, num_aperiodicities, window):
    boundary = round(fft_size / len(window) * 8)
    half_window_length = len(window) // 2
    coarse_aperiodicity = np.zeros(num_aperiodicities)
    
    for i in range(num_aperiodicities):
        center = int(frequency_interval * i / (fs / fft_size))
        segment = power_spectrum[(center - half_window_length):(center + half_window_length + 1)] * window
        spectrum = np.abs(fft(segment, fft_size)) ** 2
        cumulative_spectrum = np.cumsum(np.sort(spectrum[:fft_size // 2 + 1]))
        coarse_aperiodicity[i] = -10 * np.log10(cumulative_spectrum[fft_size // 2 - boundary] / cumulative_spectrum[-1])
...
def nuttall(N):
    ...
    coefs = [0.355768, -0.487396, 0.144232, -0.012604]
    return np.dot(np.cos(t[:, None] * np.arange(4)), coefs)
#估计语音信号的带通非周期性，用于分析语音信号的非周期成分，并在后续的语音合成中帮助生成更自然的语音