...
import tool.gcfb_v234.utils as utils
import tool.gcfb_v234.gammachirp as gc
...
def set_param(gc_param):
    if isinstance(gc_param, dict):
        gc_param = SimpleNamespace(**gc_param)
    ...
    if not hasattr(gc_param, 'hloss_type'):
        gc_param.hloss_type = 'NH_NormalHearing'
    ...
    if not hasattr(gc_param, 'dyn_hpaf'):
        ...
        if not hasattr(gc_param, 'dyn_hpaf_str_prc'):
            gc_param.dyn_hpaf_str_prc = 'frame-base'
