...
def MakeAsymCmpFiltersV2(fs, Frs, b, c):
    ...
    for Nfilt in range(NumFilt):
        ...
        denominator = np.sum(vwrs * bz, axis=1)
        denominator[denominator == 0] = np.finfo(float).eps  # 避免除零
        nrm = np.abs(np.sum(vwrs * ap, axis=1) / denominator)
